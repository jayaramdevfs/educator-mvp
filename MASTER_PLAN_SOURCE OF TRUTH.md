...
## 20. Sprint 11 -- Admin Experience, Subscriptions & Payment

**Objective:** Build the complete admin panel, implement subscription/payment backend, wire learner subscription UI and public pricing page, and fix the exam access control bug.
...
| # | Task | Details | Status |
|---|---|---|---|
| B4.1 | Add PublicExamController | GET /api/public/exams — anonymous exam browsing | COMPLETED |
| B4.2 | Add PublicSubscriptionController | GET /api/public/subscriptions/plans — pricing page | COMPLETED |
| B4.3 | Subscription plan management | COURSE_ONLY, EXAM_ONLY, COURSE_EXAM_BUNDLE types; admin CRUD | COMPLETED |
| B4.4 | Payment initiation | POST /api/learner/subscriptions/buy/{planId} — mock providerPaymentId | COMPLETED |
| B4.5 | Payment webhook | POST /api/payment/webhook — confirms payment, activates subscription | COMPLETED |
| B4.6 | Subscription retrieval | GET /api/learner/subscriptions/my | COMPLETED |
| B4.7 | Payment history | GET /api/learner/payments/history | COMPLETED |
| B4.8 | Access control integration | Subscription-gated exam/course access wired | COMPLETED |
| B4.9 | SecurityConfig update | permitAll for /api/public/exams/** and /api/public/subscriptions/** | COMPLETED |
| **BUG** | **Exam access control fix** | **ExamAttemptService was using UUID.getMostSignificantBits() (random Long) instead of resolving real User PK via email. Fixed in StudentExamController + ExamAttemptService.** | **FIXED** |

### Frontend Tasks (Completed)

| # | Task | Details | Status |
|---|---|---|---|
| F4.1 | Build Admin Dashboard | Enhanced with quick-action navigation cards | COMPLETED |
| F4.2 | Build Admin Courses page | Course CRUD management table with status badges | COMPLETED |
| F4.3 | Build Admin Exams page | Exam management table with publish/archive actions | COMPLETED |
| F4.4 | Build Admin Homepage CMS | Section builder, block configurator, layout picker | COMPLETED |
| F4.5 | Build Admin Subscriptions page | Subscription plan CRUD with type and pricing | COMPLETED |
| F4.6 | Build Admin Users page | User table, search, role assignment | COMPLETED |
| F4.7 | Build Instructor Dashboard | Full course and exam management UI for instructors | COMPLETED |
| F4.8 | Build Learner Subscriptions page | Plan cards with feature list and purchase CTA | COMPLETED |
| F4.9 | Build Learner Subscription Buy page | Plan purchase flow (/learner/subscriptions/buy/[planId]) | COMPLETED |
| F4.10 | Build Public Pricing page | Plan comparison table at /pricing | COMPLETED |
| F4.11 | Wire exam taking page | Fixed 403 access-denied; passes email to API for user resolution | COMPLETED |
| F4.12 | Update top-nav | Subscription and pricing links added | COMPLETED |
| F4.13 | Update learner page | Subscription entry point and plan status widget | COMPLETED |

### Testing Tasks (Completed)

| # | Task | Details | Status |
|---|---|---|---|
| T4.1 | ExamAttemptServiceTimerShuffleTest | Updated 3 tests for new 3-arg startAttempt signature | COMPLETED |
| T4.2 | SecurityIntegrationTest | Updated to cover new public route permissions | COMPLETED |
| T4.3 | Newman E2E: subscription flow | Login -> buy -> webhook -> validate ACTIVE subscription | COMPLETED |
| T4.4 | Newman E2E: payment history | GET /api/learner/payments/history verified | COMPLETED |

### Exit Criteria

- [x] Admin can view courses, exams, homepage sections, subscriptions, users
- [x] Instructor has a functional dashboard
- [x] Learner can browse subscription plans and initiate purchase
- [x] Public pricing page available without login
- [x] Exam start 403 bug fixed — students can take exams
- [x] Subscription purchase, webhook confirmation, and activation work
- [x] Payment history retrieval works
- [x] 155 backend tests passing, frontend build clean (25 routes)
- [x] CI green

**Sprint 11 Status: COMPLETED AND CLOSED**

**Closure Reports:** `docs/reports/sprint11/SPRINT_11_CLOSURE.md`, `docs/reports/sprint11/SPRINT_11_BACKEND_CLOSURE.md`
...
### FE-001: Subscription & Payment System (Sprint 11 Scope Expansion)

| Field | Value |
|---|---|
| ID | FE-001 |
| Title | Subscription Plans, Purchase Flow, Payment Webhook |
| Status | IMPLEMENTED |
| Priority | P1 |
| Sprint | 11 |
| Proposed Date | 2026-02-16 |
| Implemented Date | 2026-02-17 |

**Description:** Added complete subscription and payment backend to Sprint 11 scope. Originally deferred (see Section 9.1), but moved forward because the frontend admin experience required subscription plan management, and the learner experience required a pricing/purchase flow.

**Acceptance Criteria:**
- [x] Subscription plans with type (COURSE_ONLY, EXAM_ONLY, COURSE_EXAM_BUNDLE)
- [x] Purchase flow: initiate -> providerPaymentId -> webhook -> activate
- [x] Subscription activation auto-enrolls student in attached courses
- [x] Payment history API
- [x] Public pricing page backed by real plan data
- [x] Learner subscription management page

**Impact:** Added `subscription/` package, `payment/` webhook controller, `PublicSubscriptionController`. Frontend: 3 new pages (pricing, subscriptions, buy flow).

---

### FE-002: Exam Access Control Bug Fix

| Field | Value |
|---|---|
| ID | FE-002 |
| Title | Fix ExamAttemptService UUID->Long User ID Resolution |
| Status | IMPLEMENTED |
| Priority | P0 |
| Sprint | 11 |
| Proposed Date | 2026-02-16 |
...
**Description:** Critical bug — `ExamAttemptService.startAttempt()` called `UUID.getMostSignificantBits()` to extract a `Long` user ID. This produces a random Long (bitwise extraction) that never matches any real `User.id` (DB auto-increment Long), causing every exam start attempt to return 403.

**Fix:** `StudentExamController` passes authenticated email; `ExamAttemptService` resolves real `Long` PK via `userRepository.findByEmail(email)`.

**Impact:** `StudentExamController.java`, `ExamAttemptService.java`, `ExamAttemptServiceTimerShuffleTest.java` (3 tests updated for new signature).

---

### FE-003: Public Exam Browsing Endpoint

| Field | Value |
|---|---|
| ID | FE-003 |
| Title | PublicExamController — Anonymous Exam Browsing |
| Status | IMPLEMENTED |
| Priority | P2 |
| Sprint | 11 |
| Proposed Date | 2026-02-16 |
...
**Description:** New `PublicExamController` at `/api/public/exams` allows unauthenticated users to browse published exams. Feeds the pricing and catalog pages.

**Impact:** New file `PublicExamController.java`. SecurityConfig updated to `permitAll` for `/api/public/exams/**`.
...
| 2026-02-17 | 2.2 | Sprint 11 completed and closed | Admin experience pages, subscription/payment backend, exam access control bug fix (FE-001, FE-002, FE-003). 155 backend tests, 25 frontend routes, CI green. Sprint 12 is next. |
