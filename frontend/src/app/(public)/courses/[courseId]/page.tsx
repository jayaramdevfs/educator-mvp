...
async function fetchLessonTree(courseId: string): Promise<LessonTreeNode[]> {
  try {
    const response = await fetch(`${API_BASE_URL}/api/public/lesson-tree/course/${courseId}`, {
      cache: "no-store",
    });
      ...
      {nodes.map((node) => (
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <span className="text-xs text-slate-500">Depth {node.depthLevel}</span>
          </div>
          {node.children.length > 0 && (
            <div className="mt-2 border-l border-slate-700 pl-3">{renderTree(node.children)}</div>
          )}
        </li>
      ))}
    </ul>
...
export default async function CourseDetailPage({
  ...
  const [course, tree] = await Promise.all([fetchCourse(courseId), fetchLessonTree(courseId)]);
        ...
        <div className="relative mx-auto max-w-6xl space-y-6">
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-2xl shadow-purple-500/10 backdrop-blur-xl">
            ...
            <p className="mt-3 text-slate-400">
              {course.descriptionEn || "No description available for this course yet."}
            </p>

            <div className="mt-4 flex flex-wrap gap-4 text-sm text-slate-400">
              <span>Language: {course.languageCode}</span>
              <span>Duration: {course.estimatedDurationMinutes} mins</span>
            </div>

            <div className="mt-6 flex gap-3">
              <Link
                href="/register"
                className="rounded-lg bg-gradient-to-r from-purple-600 to-purple-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-purple-500/30 transition-all hover:from-purple-500 hover:to-purple-400"
              >
                Enroll Now
              </Link>
              <Link
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6 shadow-xl shadow-purple-500/10 backdrop-blur-xl">
            <h2 className="text-xl font-semibold text-slate-100">Lesson Tree</h2>
